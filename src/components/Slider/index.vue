...
const calcState = () => {
  ...
  const { state } = sliderItems.value.reduce((a, b) => (a.state.left > b.state.left ? a : b))

  const { left, width } = state

  maxScroll.value = x.value * -1 + (x.value * -1 + left - window.innerWidth / 2 + width / 2)
}
...
const { pause, resume } = useRafFn(() => {
  ...
  const position = Number(interpolate(positionX.value, x.value, 0.08).toFixed(2))
  ...
  activeIndex.value = sliderItems.value.findIndex((item) => item.progress === closest)
})
</script>
